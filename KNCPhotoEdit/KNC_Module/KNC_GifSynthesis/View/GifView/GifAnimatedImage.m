...
@optional
- (void)debug_animatedImage:(GifAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(GifAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(GifAnimatedImage *)animatedImage;
@end
#endif
...
@property (nonatomic, strong, readonly) GifAnimatedImage *weakProxy;
...
@implementation GifAnimatedImage
...
+ (void)initialize
{
    if (self == [GifAnimatedImage class]) {
...
- (instancetype)init
{
    GifAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
...
{
    GifAnimatedImage *animatedImage = [[GifAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    GifAnimatedImage * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[GifAnimatedImage class]]) {
        GifAnimatedImage *animatedImage = (GifAnimatedImage *)image;
...
@implementation GifAnimatedImage (Logging)
